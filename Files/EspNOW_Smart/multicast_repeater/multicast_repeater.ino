...
String peer_address [2] = {"bedroom_adress", " repeater2_adress"};
...
void esp_smart_add_peer(String peer_addr_list[],int num_addr, int peer_channel, bool encrypt){
 while(count <= num_addr){
  memcpy(peerInfo.peer_addr, peer_addr_list[count], 6);
  peerInfo.channel = peer_channel;
  peerInfo.encrypt = encreypt;
  if (esp_now_add_peer(&peerInfo) != ESP_OK) {
    Serial.println("Failed to add peer");
    return;
  }
